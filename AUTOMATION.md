...
| `live-health-check.yml` | 6시간마다 | 전 페이지 200 OK + 핵심 메타 존재 확인 | 🐙 **GitHub 이슈 자동생성**(→오너 메일, 시크릿 불필요) + 복구 시 자동 종료 / 📧 이메일(시크릿 있으면) |
...
> 시크릿이 없어도 워크플로는 정상 동작합니다. 특히 **라이브 장애 알림은 시크릿 없이도**
> GitHub 이슈로 자동 생성되어 오너(theassetsquare@gmail.com)에게 메일로 통지되고,
> 사이트가 복구되면 이슈가 자동으로 닫힙니다. SMTP 이메일은 더 풍부한 알림용(선택).
